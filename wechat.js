...
console.log("PHONE_NUMBER_ID pconst express = require("express");
const axios = require("axios");
const { initDB, clearHistory } = require("./db");
const { askClaudeWithMemory } = require("./askClaude-memory");
...
async function processMessage(userId, userText, sendFn, platform) {
  ...
  if (lowerText === "reset") {
    await clearHistory("wechat", userId);
    await sendFn("Fresh start! What can I help you with? 😊");
    return;
  }

  const reply = await askClaudeWithMemory("wechat", userId, userText, SYSTEM_PROMPT);
  await sendFn(reply);
